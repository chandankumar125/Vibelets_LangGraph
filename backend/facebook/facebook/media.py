...
async def list_media(
    ...
    """
    List uploaded images and videos for an ad account
    """
    try:
        async with httpx.AsyncClient() as client:
            ...
            if images_response.status_code == 200:
                ...
                if "data" in images_data:
                    for img in images_data["data"]:
                        media_items.append({
                            "id": img.get("hash", ""),
                            "url": img.get("url", ""),
                            "type": "image",
                            "name": img.get("name", "Untitled Image")
                        })
            ...
            if videos_response.status_code == 200:
                ...
                if "data" in videos_data:
                    for vid in videos_data["data"]:
                        media_items.append({
                            "id": vid.get("id", ""),
                            "url": vid.get("picture", ""),
                            "type": "video",
                            "name": vid.get("title", "Untitled Video")
                        })
