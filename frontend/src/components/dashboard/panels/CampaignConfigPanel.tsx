import { CampaignConfig, CreativeOption, AdAccount, FacebookPage } from '@/types/campaign';
import { Settings, Target, DollarSign, MousePointer, Calendar, Facebook, Check, Play, Loader2, FileText, Layers, Flag } from 'lucide-react';
          ...
          <div className="rounded-lg border border-border bg-card overflow-hidden">
            ...
            <div className="p-2">
              ...
              <div className={cn(
                ...
                <div className={cn(
                  "w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0",
                  selectedPage ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"
                )}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn(
                    "text-[11px] font-medium truncate",
                    selectedPage ? "text-foreground" : "text-muted-foreground"
                  )}>
                    {selectedPage?.name || 'Facebook Page'}
                  </p>
                  <p className="text-[9px] text-muted-foreground">
                    {selectedPage ? 'Page Selected' : 'Waiting for selection...'}
                  </p>
                </div>
                {selectedPage ? (
                  <Check className="w-3.5 h-3.5 text-primary flex-shrink-0" />
